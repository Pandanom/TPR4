...
namespace TPR4
{
    class Equation
    {
        ...
        public struct Right
        {
            public bool Sign; public double value;
        ...
        public void FindDbr()
        {
            for(int i = 0; i < startXNum; i++)
            dbrFree.Add(i+1, 0);
           
            for (int i = 0; i < rows; i++)
                for (int j = startXNum; j < eq[i].Count; j++)
                    if (eq[i][j] < 0)
                        dbrFree.Add(j + 1, 0);
                    else if (eq[i][j] > 0)
        ...
        public void Simplex()
        {
            ...
            while ((f.max && s[col] < new MNum()) || (!f.max && s[col] > new MNum())&& qwe > 0);
            bi_a = dbrB;
        ...
        public void PrintAnswer()
        {
            ...
            Console.WriteLine(" -> " + (f.max ? "max" : "min"));
            string st = "";
            Answer = new double[startXNum];
            for (int i = 1; i < startXNum; i++)
            {
                st += bi_a.ContainsKey(i) ? bi_a[i].ToString() + ", " : "0, ";
                Answer[i-1] = bi_a.ContainsKey(i) ? bi_a[i].ToDouble() : 0;
            }
            st += bi_a.ContainsKey(startXNum) ? bi_a[startXNum].ToString() : "0";
            Answer[startXNum - 1] = bi_a.ContainsKey(startXNum) ? bi_a[startXNum].ToDouble() : 0;
